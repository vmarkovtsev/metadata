...
func (db *Database) UpsertRepository(ctx context.Context, event *gh.RepositoryEvent) (err error) {
	defer errRecover(event, &err)

	switch event.GetAction() {
	case "deleted", "anonymous_access_enabled", "anonymous_access_disabled":
		break
...
func (db *Database) UpsertOrganization(ctx context.Context, event *gh.OrganizationEvent) (err error) {
	defer errRecover(event, &err)
	...
	case "created", "renamed", "member_added", "member_invited":
		...
		return db.txExecContext(ctx, query,
			sum256(event.GetOrganization().GetID()), // sum256,
			pq.Array([]int64{ver}),                  // versions,
...
func (db *Database) UpsertPullRequest(ctx context.Context, event *gh.PullRequestEvent) (err error) {
	defer errRecover(event, &err)
	...
	case "assigned",
		...
		return db.txExecContext(ctx, query,
			sum256(
				event.GetRepo().GetOwner().GetID(),
				event.GetRepo().GetID(),
				event.GetPullRequest().GetID(),
				event.GetPullRequest().GetUser().GetID(),
			), // sum256,
			pq.Array([]int64{ver}),                       // versions,
			...
			event.GetRepo().GetName(),                    // repository_name text NOT NULL,
			event.GetRepo().GetOwner().GetLogin(),        // repository_owner text NOT NULL,
			event.GetRepo().GetFullName(),                // repository_fullname
...
func (db *Database) UpsertPullRequestReviewComment(ctx context.Context, event *gh.PullRequestReviewCommentEvent) (err error) {
	defer errRecover(event, &err)
...
func (db *Database) UpsertPullRequestReview(ctx context.Context, event *gh.PullRequestReviewEvent) (err error) {
	defer errRecover(event, &err)
...
func (db *Database) UpsertIssues(ctx context.Context, event *gh.IssuesEvent) (err error) {
	defer errRecover(event, &err)
	...
	case "opened",
		...
		return db.txExecContext(ctx, query,
			...
			event.GetRepo().GetName(),             // repository_name text NOT NULL,
			event.GetRepo().GetOwner().GetLogin(), // repository_owner text NOT NULL,
			event.GetRepo().GetFullName(),         // repository_fullname text NOT NULL,
...
func (db *Database) UpsertIssueComment(ctx context.Context, event *gh.IssueCommentEvent) (err error) {
	defer errRecover(event, &err)
	...
	case "created", "edited":
		if event.GetIssue().IsPullRequest() {
			err = db.upsertIssueCommentAsPullRequest(ctx, event)
		} else {
			err = db.upsertIssueComment(ctx, event)
		}
	}
...
func (db *Database) upsertIssueComment(ctx context.Context, event *gh.IssueCommentEvent) error {
	const tab = "github_issue_comments_versioned"
	cols := tables[tab]
	ver := version()
	comment, issue := event.GetComment(), event.GetIssue()
	...
	return db.txExecContext(ctx, query,
		...
		event.GetRepo().GetName(),             // repository_name,
		event.GetRepo().GetOwner().GetLogin(), // repository_owner,
		event.GetRepo().GetFullName(),         // repository_fullname text NOT NULL,
...
func (db *Database) upsertIssueCommentAsPullRequest(ctx context.Context, event *gh.IssueCommentEvent) (err error) {
	const tab = "github_pull_request_comments_versioned"
	cols := tables[tab]
	ver := version()
	comment := event.GetComment()
	...
	return db.txExecContext(ctx, query,
		...
		pq.Array([]int64{ver}),                // versions,
		event.GetIssue().IsPullRequest(),      // author_association text,
		comment.GetBody(),                     // body text,
		"",                                    // commit_id text,
		comment.GetCreatedAt(),                // created_at timestamptz,
		"",                                    // diff_hunk text,
		...
		"",                                    // path text,
		"",                                    // position bigint,
		event.GetIssue().GetNumber(),          // pull_request_number bigint NOT NULL,
